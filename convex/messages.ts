...
const populateThread = async (ctx: QueryCtx, messageId: Id<"messages">) => {
    const messages = await ctx.db.query("messages")
        .withIndex("by_parent_message_id", (q) =>
            q.eq("parentMessageId", messageId)).collect()
...
const populateReactions = (ctx: QueryCtx, messageId: Id<"messages">)=> {
    return ctx.db.query("reactions")
        .withIndex("by_message_id", (q) => {
            return q.eq("messageId", messageId)
        }).collect()
}
...
export const get = query({
    args: {
        ...
        if (!args.conversationId && !args.conversationId && args.parentMessageId) {
        ...
        const results = await ctx.db
            ...
            .withIndex("by_channel_id_parent_message_id_conversation_id", (q) => {
                return q.eq("channelId", args.channelId)
